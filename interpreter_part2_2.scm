...
(define m-state-stmt-list
  (lambda (stmt-list s return break continue throw-break throw)
    (if (null? stmt-list) s
        (m-state-stmt-list (cdr stmt-list) (m-state (car stmt-list) s return break continue throw-break throw) return break continue throw-break throw)))) ; what if we get break outside of {}
...
(define m-state
  (lambda (stmt s return break continue throw-break throw)
    (cond
      ((null? stmt) s)
      ((not (list? stmt)) s)
      ...
      ((assignment? stmt) (m-state-assign (assignment-variable stmt) (assignment-expression stmt) s return break continue throw-break throw))
      ((ifthenelse? stmt) (m-state-if (if-condition stmt) (then-statement stmt) (else-statement stmt) s return break continue throw-break throw))
      ((ifthen? stmt) (m-state-if (if-condition stmt) (then-statement stmt) '() s return break continue throw-break throw))
      ((while? stmt) (m-state-while (while-condition stmt) (while-body-statement stmt) s return throw-break throw))
      ;((return? stmt) (m-state-return (return-expression stmt) s))
      ((unary? stmt) (m-state (operand1 stmt) s return break continue throw-break throw))
      ((bool-exp? stmt) (m-state (operand2 stmt) (m-state (operand1 stmt) s return break continue throw-break throw) return break continue throw-break throw))
      ((int-exp? stmt) (m-state (operand2 stmt) (m-state (operand1 stmt) s return break continue throw-break throw) return break continue throw-break throw))
      (else
       s))))
...
(define m-state-declare
  (lambda (variable s return break continue throw-break throw)
    (cond
...
(define m-state-assign
  (lambda (variable exp s return break continue throw-break throw)
    (cond
      ((not (declared? variable (m-state exp s return break continue throw-break throw))) (error 'UndeclaredVariable))
      (else
       (removeadd variable (m-value-exp exp s return break continue throw-break throw) s)))))
...
(define m-state-if
  (lambda (if-cond then-stmt else-stmt s return break continue throw-break throw)
    (cond
      ((m-value-exp if-cond (m-state if-cond s return break continue throw-break throw) return break continue throw-break throw)
       (m-state then-stmt (m-state (operand2 if-cond) (m-state (operand1 if-cond) s return break continue throw-break throw) return break continue throw-break throw) return break continue throw-break throw))
      (else
       (m-state else-stmt (m-state (operand2 if-cond) (m-state (operand1 if-cond) s return break continue throw-break throw) return break continue throw-break throw) return break continue throw-break throw)))))

(define m-state-while
  (lambda (while-cond loop-body s return throw-break throw)
    (call/cc
     (lambda (break)
        (m-state-while-loop while-cond loop-body s return break break throw-break throw))))) ; need to remove top layer if we breaked inside {}

; loop body can contain block stmt
(define m-state-while-loop
  (lambda (while-cond loop-body s return break continue throw-break throw)
    (cond
...
(define m-value-exp
  (lambda (exp s return break continue throw-break throw)
    (cond
      ((int-exp? exp) (m-value-int exp s return break continue throw-break throw))
      ((bool-exp? exp) (m-value-bool exp s return break continue throw-break throw))
      ((not (list? exp)) (m-value-variable exp s return break continue throw-break throw))
      (else
       (m-value-variable (operand1 exp) (m-state exp s return break) return break continue throw-break throw)))))
...
(define m-value-variable
  (lambda (variable s return break continue throw-break throw)
    (cond
      ...
      ((null? (car (car-slayer s))) (m-value-variable variable (cdr-slayer s) return break continue throw-break throw))
      ((eq? (carvar (car-slayer s)) variable) (carval (car-slayer s)))
      (else
       (m-value-variable variable (cons (cdrstate (car-slayer s)) (cdr-slayer s)) return break continue throw-break throw)))))
...
(define m-value-int
  (lambda (exp s return break continue throw-break throw)
    (cond
      ((null? exp) 0)
      ((number? exp) exp) ; '6
      ((eq? (operator exp) '+) (m-value-binaryop + exp s return break continue throw-break throw))
      ((subtraction? exp) (m-value-binaryop - exp s return break continue throw-break throw))
      ((negative? exp) (m-value-unaryop - exp s return break continue throw-break throw))
      ((eq? (operator exp) '*) (m-value-binaryop * exp s return break continue throw-break throw))
      ((eq? (operator exp) '/) (m-value-binaryop quotient exp s return break continue throw-break throw))
      ((eq? (operator exp) '%) (m-value-binaryop remainder exp s return break continue throw-break throw))
...
(define m-value-bool
  (lambda (exp s return break continue throw-break throw)
    (cond
      ...
      ((eq? exp 'true) #t)
      ((eq? exp 'false) #f)
      ((eq? (operator exp) '&&) (m-value-booleanop myand exp s return break continue throw-break throw))
      ((eq? (operator exp) '||) (m-value-booleanop myor exp s return break continue throw-break throw))
      ((eq? (operator exp) '==) (m-value-booleanop eq? exp s return break continue throw-break throw))
      ((eq? (operator exp) '!=) (m-value-booleanop noteq exp s return break continue throw-break throw))
      ((eq? (operator exp) '<) (m-value-booleanop < exp s return break continue throw-break throw))
      ((eq? (operator exp) '<=) (m-value-booleanop <= exp s return break continue throw-break throw))
      ((eq? (operator exp) '>) (m-value-booleanop > exp s return break continue throw-break throw))
      ((eq? (operator exp) '>=) (m-value-booleanop >= exp s return break continue throw-break throw))
      ((eq? (operator exp) '!) (m-value-unaryop not exp s return break continue throw-break throw))
      (else (error 'badoperation "Unknown operator")))))
...
(define m-value-binaryop
  (lambda (op exp s return break continue throw-break throw)
    (op (m-value-exp (operand1 exp) s return break continue throw-break throw) (m-value-exp (operand2 exp) (m-state (operand1 exp) s return break continue throw-break throw) return break continue throw-break throw))))
...
(define m-value-unaryop
  (lambda (op exp s return break continue throw-break throw)
    (cond
      ((number? (op (m-value-exp (operand1 exp) s return break continue throw-break throw)))
       (op (m-value-exp (operand1 exp) s return break continue throw-break throw)))
      (else
       (op (m-value-exp (operand1 exp) s return break continue throw-break throw))))))
...
(define try-catch-finally
  (lambda (try-block catch-block finally-block s return break continue throw)
    (call/cc
     (lambda (throw-break)
       (try-catch-finally-break try-block catch-block finally-block s return break continue throw-break throw)))))
